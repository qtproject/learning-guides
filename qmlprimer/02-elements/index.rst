...
Visual elements can be manipulated by changing their properties such as position, size, visibility, opacity, etc. This can be done with or without animations. There are special elements for capturing user input or visualizing data from different sources. Qt documentation provides a list of all items :qt:`sorted in groups <qml-groups.html>`. Please take some time to get an overview of all available items.
...
We could add more elements, all of them will be added to this stack and drawn on the upper left corner which is the     (0,0)* position. This is the default rendering position, if you do not specify ``x`` and ``y`` properties.
...
Though the use of the IDs is optional in Qt Quick. We have to use IDs if items have to arranged in relation to each other. Generally, you should strongly consider using IDs all the time. This greatly improves readability if the code and prevents weird side effects. Be advised to use consistent IDs for all root items in your project, for examples, just ``root``. This will help you keep track which items is used and avoid side some effects.

We already saw some use of anchoring in previous examples. Lets take a closer look on this and use it place elements so that they will start getting closer to a clock.

We just take two :qt:`Text<qml-text.html>` elements and place them inside our ``root`` element. Anchoring uses so called anchoring lines which are provided as properties. When you anchor items, you just bind an anchoring line of one item to an anchoring line of another one. You basically stitch items to each other. Additionally, you can also set anchor margins. Margins are zero but default and define a distance between anchored items. We place ``timeText`` and ``dateText`` centered inside of ``root`` and add margins of 10 pixels:
...
The item :qt:`MouseArea<qml-mousearea.html>` fills the entire surface of ``root`` and even reacts on mouse clicks received over the both text items. We will take a look on processing of mouse events later.
...
Lets try migrate our application to use positioners. It will make the code shorter even with our two items. We now place both :qt:`Text<qml-text.html>` elements inside of :qt:`Column<qml-column.html>` positioner. In order to keep them apart from each other we define 20 pixels spacing and anchor the :qt:`Column<qml-column.html>` in the center of root:
...
If you now resize the application window, you will notice that its content stays centered, since we centered the positioner in ``root``.

Qt Quick provides other :qt:`Positioner<qml-positioners.html>` elements, such as  :qt:`Row <qml-row.html>`, :qt:`Grid<qml-grid.html>` or :qt:`Flow<qml-flow.html>`. We will use some of them in the course of this guide.
...
You can define a new property in the following syntax:
...
In fact, in the case of our button inheriting from :qt:`Rectangle<qml-rectangle.html>`, we should define the new property as a new alias since the :qt:`Rectangle<qml-rectangle.html>` element already has a property defining the the color of the boarder:
...
The code of handler reacting upon changes can be much more sophisticated and use  JavaScript. We will talk about using JavaScript in Qt Quick in the section :ref:`primer-javascript-section`

Property binding is another fundamental aspect. It makes a property to follow changes of another property. This works across items, and is actually used across items most of the time. Binding occurs each time you use ``:`` and have a value on the right side which can change:
...
The latter is interesting since you can also do some calculations or even bind to a function returning a value.
...
Later on, we will use properties to define custom parameters in our application. These parameters will define the appearance of our application as well as keep a few settings.

Other visual composition elements
----------------------------------

Qt Quick 1.x provides a broad selection of items which can be used for UI creation. Qt documentation :qt:`covers them<qdeclarativeelements.html>` in full details and we are not going to discuss all of them here. In general, there four sets of elements which can be used for composing application UIs:
...
Qt5 will add two more:

     Vector-oriented graphics canvas
     3D rendering with effects and shaders

These topics are covered by other guides available :guides-homepage:`from here<>`

Even though you do not see high level UI components on the list, Qt Quick will give you more than enough power to compose very rich UIs. These UIs can like cartoons, where images will receive user input and be smoothly transformed into other images or elements, rectangles rounded to become circles, ellipses etc. You can also create classical UI components if required without significant effort. When you start working on a UI, try to first decompose it in basic elements, text and images and views. Think how elements interact with each other, what happens upon user input or changes in the application logic.
